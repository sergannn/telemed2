...
void initState() {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    medicalHistories[i] = MedicalHistorySelection(); // Добавляем для всех индексов
  }
}
...
// Загружаем список заболеваний
  ...
  medicalHistories[2]?.selectedTraumas = prefs.getString('medical_trauma_2') ?? '';
  medicalHistories[2]?.selectedOperations = prefs.getString('medical_operation_2') ?? '';
  medicalHistories[2]?.selectedMedications = prefs.getString('medical_medication_2') ?? '';

  medicalHistories[6]?.selectedCycleLength = prefs.getString('cycle_length_6') ?? '';
  medicalHistories[6]?.selectedCycleVariation = prefs.getString('cycle_variation_6') ?? '';
  medicalHistories[6]?.selectedFlowIntensity = prefs.getString('flow_intensity_6') ?? '';
  medicalHistories[6]?.selectedPainLevel = prefs.getString('pain_level_6') ?? '';

  medicalHistories[7]?.selectedBirthType = prefs.getString('birth_type_7') ?? '';
  medicalHistories[7]?.selectedAbortionType = prefs.getString('abortion_type_7') ?? '';
  medicalHistories[7]?.selectedPregnancyComplications = prefs.getString('pregnancy_complications_7') ?? '';

  for (int i = 0; i < 8; i++) {
  List<String>? savedDiseases = prefs.getStringList('chronic_diseases_$i');
  medicalHistories[i]?.selectedDiseases = savedDiseases ?? [];
  medicalHistories[i]?.selectedCycleLength = prefs.getString('cycle_length_$i') ?? '';
  medicalHistories[i]?.selectedCycleVariation = prefs.getString('cycle_variation_$i') ?? '';
  medicalHistories[i]?.selectedFlowIntensity = prefs.getString('flow_intensity_$i') ?? '';
  medicalHistories[i]?.selectedPainLevel = prefs.getString('pain_level_$i') ?? '';
}
  ...
  Future<void> _saveAllValues() async {
    ...
    await prefs.setString('physical_0', physicalLimitations[0]?.selectedValue ?? '');
    ...
    await prefs.setString('circumference_1', bodyParams[1]?.circumference ?? '');
// Сохраняем список заболеваний
  await prefs.setStringList('medical_disease_2', medicalHistories[2]?.selectedDiseases ?? []);
  
  // Сохраняем остальные значения
  await prefs.setString('medical_trauma_2', medicalHistories[2]?.selectedTraumas ?? '');
  await prefs.setString('medical_operation_2', medicalHistories[2]?.selectedOperations ?? '');
  await prefs.setString('medical_medication_2', medicalHistories[2]?.selectedMedications ?? '');

   await prefs.setString('cycle_length_6', medicalHistories[6]?.selectedCycleLength ?? '');
  await prefs.setString('cycle_variation_6', medicalHistories[6]?.selectedCycleVariation ?? '');
  await prefs.setString('flow_intensity_6', medicalHistories[6]?.selectedFlowIntensity ?? '');
  await prefs.setString('pain_level_6', medicalHistories[6]?.selectedPainLevel ?? '');

  await prefs.setString('birth_type_7', medicalHistories[7]?.selectedBirthType ?? '');
  await prefs.setString('abortion_type_7', medicalHistories[7]?.selectedAbortionType ?? '');
  await prefs.setString('pregnancy_complications_7', medicalHistories[7]?.selectedPregnancyComplications ?? '');

  for (int i = 0; i < 8; i++) {
  if (medicalHistories[i]?.selectedDiseases?.isNotEmpty ?? false) {
    await prefs.setStringList('chronic_diseases_$i', medicalHistories[i]?.selectedDiseases ?? []);
    await prefs.setString('cycle_length_$i', medicalHistories[i]?.selectedCycleLength ?? '');
  await prefs.setString('cycle_variation_$i', medicalHistories[i]?.selectedCycleVariation ?? '');
  await prefs.setString('flow_intensity_$i', medicalHistories[i]?.selectedFlowIntensity ?? '');
  await prefs.setString('pain_level_$i', medicalHistories[i]?.selectedPainLevel ?? '');
  }
}
}
  ...
  Widget _buildRhFactorButtons(int index) {
    return Wrap(
      ...
      runSpacing: 8.0,
      children: [
        ...['Положительный', 'Отрицательный', 'Не знаю'].map((value) =>
          GestureDetector(
            onTap: () => setState(() {
  ...
  Widget _buildDisabilityButtons(int index) {
    return Wrap(
      ...
      runSpacing: 8.0,
      children: [
        ...['1 группа', '2 группа', '3 группа', 'нет'].map((value) =>
          GestureDetector(
            onTap: () => setState(() {
  ...
  Widget _buildBodyParameters(int index) {
    return Column(
      ...
      children: [
        TextField(
          controller: TextEditingController(text: bodyParams[index]?.weight ?? ''),
        ...
        SizedBox(height: 12),
        TextField(
          controller: TextEditingController(text: bodyParams[index]?.height ?? ''),
          decoration: InputDecoration(
            hintText: 'Рост',
            border: OutlineInputBorder(
              borderRadius: BorderRadius.circular(8),
        ...
        SizedBox(height: 12),
        TextField(
          controller: TextEditingController(text: bodyParams[index]?.circumference ?? ''),
          decoration: InputDecoration(
            hintText: 'Окружность',
            border: OutlineInputBorder(
              borderRadius: BorderRadius.circular(8),
...
Widget _buildDiseasesButtons(int index) {
  return Wrap(
    ...
    runSpacing: 8.0,
    children: [
      ...['Ветрянка', 'Краснуха', 'Корь', 'Инсульт', 'Инфаркт', 'Онкология', 
          'Менингит', 'Панкреатит', 'Ревматизм'].map((value) =>
        InkWell(
          onTap: () => setState(() {
            List<String> currentDiseases = medicalHistories[index]?.selectedDiseases ?? [];
          ...
          child: Container(
            padding: EdgeInsets.symmetric(horizontal: 12, vertical: 8),
            decoration: BoxDecoration(
              color: medicalHistories[index]?.selectedDiseases.contains(value) ?? false
                  ? Colors.blue.withOpacity(0.2)
                  : Colors.transparent,
              border: Border.all(
                color: medicalHistories[index]?.selectedDiseases.contains(value) ?? false
      ...
      ...['Да', 'Нет'].map((value) =>
        GestureDetector(
          ...
          child: Container(
            padding: EdgeInsets.symmetric(horizontal: 12, vertical: 8),
            decoration: BoxDecoration(
              color: index == 0 && medicalHistories[2]?.selectedTraumas == value ||
                     index == 1 && medicalHistories[2]?.selectedOperations == value ||
                     index == 2 && medicalHistories[2]?.selectedMedications == value
                  ? Colors.blue.withOpacity(0.2)
                  : Colors.transparent,
              border: Border.all(
                color: index == 0 && medicalHistories[2]?.selectedTraumas == value ||
      ...
      return AlertDialog(
        ...
        content: Container(
          ...
          child: ListView(
            ...
            children: [
              ...['Ветрянка', 'Краснуха', 'Корь', 'Инсульт', 'Инфаркт', 'Онкология',
                'Менингит', 'Острый миокардит', 'Панкреатит', 'Ревматизм', 'Тромбоз',
                'Туберкулез', 'Другое'].map((disease) => ListTile(
                  title: Text(disease),
                  onTap: () => setState(() {
                    List<String> currentDiseases = medicalHistories[index]?.selectedDiseases ?? [];
...
Widget _buildCycleLengthButtons(int index) {
  return Wrap(
    ...
    runSpacing: 8.0,
    children: [
      ...['24 дня', '24-38 дня', 'Более 38 дней', 'месячных нет'].map((value) =>
        GestureDetector(
          onTap: () => setState(() {
...
Widget _buildCycleVariationButtons(int index) {
  return Wrap(
    ...
    runSpacing: 8.0,
    children: [
      ...['Нет', '1-9 дней', 'Более 9 дней', 'Не знаю'].map((value) =>
        GestureDetector(
          onTap: () => setState(() {
          ...
          child: Container(
            padding: EdgeInsets.symmetric(horizontal: 12, vertical: 8),
            decoration: BoxDecoration(
              color: medicalHistories[index]?.selectedCycleVariation == value
                  ? Colors.blue.withOpacity(0.2)
                  : Colors.transparent,
              border: Border.all(
                color: medicalHistories[index]?.selectedCycleVariation == value
...
Widget _buildFlowIntensityButtons(int index) {
  return Wrap(
    ...
    runSpacing: 8.0,
    children: [
      ...['Скудные', 'Умеренные', 'Обильные'].map((value) =>
        GestureDetector(
          onTap: () => setState(() {
...
Widget _buildPainLevelButtons(int index) {
  return Wrap(
    ...
    runSpacing: 8.0,
    children: [
      ...['Нет', 'Почти не ощущаю', 'Средняя', 'Сильная'].map((value) =>
        GestureDetector(
          onTap: () => setState(() {
...
Widget _buildBirthTypeButtons(int index) {
  return Wrap(
    ...
    runSpacing: 8.0,
    children: [
      ...['Своевременные', 'Преждевременные', 'Не было'].map((value) =>
        GestureDetector(
          onTap: () => setState(() {
...
Widget _buildAbortionTypeButtons(int index) {
  return Wrap(
    ...
    runSpacing: 8.0,
    children: [
      ...['Да (один)', 'Да (несколько)', 'Не было'].map((value) =>
        GestureDetector(
          onTap: () => setState(() {
...
Widget _buildPregnancyComplicationsButtons(int index) {
      ...
      ...['Да', 'Нет'].map((value) =>
        GestureDetector(
          ...
          child: Container(
            padding: EdgeInsets.symmetric(horizontal: 12, vertical: 8),
            decoration: BoxDecoration(
              color: medicalHistories[index]?.selectedPregnancyComplications == value
                  ? Colors.blue.withOpacity(0.2)
                  : Colors.transparent,
              border: Border.all(
                color: medicalHistories[index]?.selectedPregnancyComplications == value
        ...
        if (index == 0)
          Column(
            ...
            children: [
              ...
              SizedBox(height: 8),
              ListTile(title: Text('Резус фактор',
              style: TextStyle(
                    fontSize: 14,
                    fontWeight: FontWeight.bold,
                    color: Colors.black87,
              ...
              SizedBox(height: 8),
              ListTile(title: Text('Инвалидность',
              style: TextStyle(
                    fontSize: 14,
                    fontWeight: FontWeight.bold,
                    color: Colors.black87,
              ...
              ListTile(title: Text('Физические ограничения',
              style: TextStyle(
                    fontSize: 14,
                    fontWeight: FontWeight.bold,
                    color: Colors.black87,
  ...
  Padding(
    ...
    child: Column(
      ...
      children: [
        ListTile(
          title: Text('Какие заболевания вы перенесли?',
          style: TextStyle(
            fontSize: 14,
            fontWeight: FontWeight.bold,
            color: Colors.black87,
        ...
        SizedBox(height: 8),
        ListTile(title: Text('Вы перенесли травмы?',
        style: TextStyle(
          fontSize: 14,
          fontWeight: FontWeight.bold,
          color: Colors.black87,
        ...
        SizedBox(height: 8),
        ListTile(title: Text('Вы перенесли операции?',
        style: TextStyle(
          fontSize: 14,
          fontWeight: FontWeight.bold,
          color: Colors.black87,
        ...
        SizedBox(height: 8),
        ListTile(title: Text('Вы принимаете лекарства на постоянной основе?',
        style: TextStyle(
          fontSize: 14,
          fontWeight: FontWeight.bold,
          color: Colors.black87,
  ...
  Padding(
    ...
    child: Column(
      ...
      children: [
        ...
        if (medicalHistories[index]?.selectedDiseases?.isNotEmpty ?? false)
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: [
              ...medicalHistories[index]?.selectedDiseases?.map((disease) => 
                Container(
                  padding: EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                  decoration: BoxDecoration(
                    color: Colors.blue.withOpacity(0.2),
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      InkWell(
                        onTap: () => setState(() {
                          medicalHistories[index]?.selectedDiseases?.remove(disease);
                        }),
